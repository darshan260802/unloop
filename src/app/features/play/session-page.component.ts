...
import {PocketPostComponent} from '../pocket-post/pocket-post.component';
import {StencilComponent} from '../stencil-studio/stencil.component';

const isChoice=(value:string|null):value is SessionChoice=>value==='random'||GAME_CARDS.some((game)=>game.id===value);

@Component({selector:'app-session-page',imports:[TuiButton,PocketPostComponent,StencilComponent,HarbourComponent,OrbitComponent],styleUrl:'./session-page.component.less',templateUrl:'./session-page.component.html'})
export class SessionPageComponent{
  ...
  constructor(){
    ...
    effect(()=>{const current=this.session.current();if(current&&current.phase==='playing'&&!this.storage.preferences().seenTutorials.includes(current.currentGame)&&this.tutorialGame()!==current.currentGame){this.tutorialGame.set(current.currentGame);this.demoDone.set(false);this.session.pause()}})
  }
  ...
  protected tutorialText(id:GameId):string{switch(id){case'pocket-post':return'Tap neighbouring squares to draw a route from bike to flag.';case'stencil-studio':return'Choose, rotate, and apply a mask to match the postcard.';case'little-harbour':return'Turn the junctions, then launch the first waiting boat.';case'orbit-garden':return'Select a ring and rotate it until all channels connect.'}}
